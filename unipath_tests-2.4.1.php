...
function _cu1rsor_assertEqu($tree, $lv = 0, $cursor_cmd = null, $cursor_arg1 = null) {
	$assert = 'return $cursor_cmd == "set";';
	if(!eval($assert)) { trigger_error($assert, E_USER_WARNING); return false; }
	...
	if(!is_array($test)) {
		$assert = 'return $data == $test; /* '.$test_name.print_r($data, true).' != '.print_r($test, true).' */';
		if(!eval($assert)) { trigger_error($assert, E_USER_WARNING); return false; }
		else return true;
	}
	...
	if(isset($test)) {
		$assert = 'return isset($data);';
		if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	}
...
//var_dump("-------", array_intersect_assoc($val, $test_val), $test_val, $val, array_intersect($val, $test_val) == $test_val);
		...
		if(is_null($found)) {
			$assert = "return in_array(\$test['$test_key'], \$data); /* \n--- $test_name NOT FOUND ---\n ".print_r($test_val, true)."\n --- BY KEY '$test_key' IN --- \n".print_r($data, true).' */';
			if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
		}
		else
			$skip[] = $found;
...
if(isset($_GET['test_parseUniPath'])) {
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr1'] == array(
			'left' => 1, 
			'left_type' => 'number', 
			'op' => '+', 
			'right' => 'expr2', 
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr2'] == array(
			'left' => 2,
			'left_type' => 'number',
			'op' => '*',
			'next' => 'expr1',
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr3'] == array(
			'left' => 'expr1',
			'left_type' => 'expr',
			'op' => '>',
			'next' => 'expr5',
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr4'] == array(
			'left' => 'expr3',
			'left_type' => 'expr',
			'op' => 'and',
			'next' => null,
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr5'] == array(
			'left' => '@obj_id',
			'left_type' => 'name',
			'op' => '=',
			'next' => 'expr4',
...
// $GLOBALS['unipath_debug_parse'] = false;
	$assert = "return \$tree[1]['filter'] == array(
		'start_expr' => 'expr3',
		'expr1' => array(
			'braket_level' => 0,
			'left' => 1, 
...
// print_r($tree[1]);
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug_parse'] = true;
	$unipath = "objs[(field1=1 or field2=2) and field3=3]";
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
// var_export($tree[1]['filter']);
	$assert = "return \$tree[1]['filter']['expr1'] == array(
			'braket_level' => 1,
			'left' => 'field1',
			'left_type' => 'name',
			...
			'right' => '1',
			'right_type' => 'number',
			'next' => 'expr3'); /* ".print_r($tree[1]['filter']['expr1'], true).'*/ ';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr2'] == array (
			'left' => 'expr1',
			'left_type' => 'expr',
			...
			'right' => 'expr3',
			'right_type' => 'expr',
		); /* ".print_r($tree[1]['filter']['expr2'], true).'*/ ';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr3'] == array (
			'left' => 'field2',
			'left_type' => 'name',
			...
			'right' => '2',
			'right_type' => 'number',
		); /* ".print_r($tree[1]['filter']['expr3'], true).'*/ ';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tree[1]['filter']['expr4'] == array (
			'left' => 'expr2',
			'left_type' => 'expr',
			...
			'right' => 'expr5',
			'right_type' => 'expr',
		); /* ".print_r($tree[1]['filter']['expr4'], true).'*/ ';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
		$assert = "return \$tree[1]['filter']['expr5'] == array (
			'left' => 'field3',
			'left_type' => 'name',
			...
			'right' => '3',
			'right_type' => 'number',
		); /* ".print_r($tree[1]['filter']['expr5'], true).'*/ ';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug_parse'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[7]['filter'] == array(
			'start_expr' => 'expr1',
			'expr1' => array('left' => 
				'@data_type', 
				'left_type' => 'name', 
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = 'return $tree[1]["name"] == "db1"; /* '.$tree[1]["name"].' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	_cu1rsor_assertEqu(
...
// print_r($tree[2]["filter"]);
	$assert = 'return $tree[2]["filter"] == array(
    "start_expr" => "expr1",
    "expr1" => array(
			"braket_level" => 0,
            "left" => "item_id",
	...
	if(!eval($assert)) trigger_error($assert."\n".print_r($tree[2]["filter"]), E_USER_WARNING);
...
/*	_cu1rsor_assertEqu(
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = 'return $tree[1] == array("name" => "cms3_object_content",
            "filter" => Array
                (
                    "start_expr" => "expr1",
                    "expr1" => Array
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return count($tree) == 3;';
	if(!eval($assert)) trigger_error($assert."\n".print_r($result), E_USER_WARNING);
  ...
  'unipath' => 'db1/alias(\\'table1\\',\\'tbl1\\')[id=1]+alias(\\'table2\\',\\'tbl2\\')[tbl1.id=tbl2.id]
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[3] == array('name' => \"columns(chunked(table1.Data, 10000, 3000), alias('table1.Id', 'id1'), 'table1.ContentType_id, REPLACE(''abcd)asd'', '')'', ''('')')\",
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[2] == array('name' => 'orders',
			'filter' => array('start_expr' => 'expr1',
				'expr1' => array(
					'braket_level' => 0,
					'left' => 'u_id',
...
//$GLOBALS['unipath_debug'] = true;
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[2] == array('name' => 'table1',
			'filter' => array('start_expr' => 'expr1',
				'expr1' => array(
					'braket_level' => 0,
					'left' => 'id',
...
// $GLOBALS['unipath_debug_parse'] = true;
	$tree = __uni_parseUniPath($unipath);
// $GLOBALS['unipath_debug_parse'] = false;
	$assert = "return \$tree[1] == array('name' => '.',
			'filter' => array('start_expr' => 'expr1',
				'expr1' => array(
					'braket_level' => 0,
					'left' => 1,
...
// var_export($tree[4]);
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[10] == array('name' => 'saveAs(`/tmp/$fs_filename`)', 'unipath' => '_SERVER/DOCUMENT_ROOT/asDirectory()/objs/123/Pictures/$fs_filename/asImageFile()/resize(`100`, `100`, `inbox`)/saveAs(`/tmp/$fs_filename`)');";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// print_r($tree);
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[1] == array('name' => 'file://', /* 'data' => 'file://', 'data_type' => 'string/local-filesystem', 'data_tracking' => array('key()' => 'file://'),*/ 'unipath' => 'file://'); /* ".print_r($tree[1], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $tree[2] == array("name" => ".", "unipath" => "file://."); /* '.print_r($tree[1], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	echo "<h3>--- $unipath ---</h3>";
	$tree = __uni_parseUniPath($unipath);
	$assert = "return \$tree[7] == array('name' => '$fs_filename', 'unipath' => '_SERVER/DOCUMENT_ROOT/asDirectory()/objs/123/Pictures/\'$fs_filename\''); /* ".print_r($tree[7], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// print_r($tree);
	...
	$assert = "return \$result == array(array(0 => '/tmp/$fs_filename'), array(0 => 'string')); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// print_r($result);
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = "return \$result[2] == array('name' => 'regexp_all(`<a.*?href=[\'\"]([^\"\']+)`)', 'unipath' => '/content/regexp_all(`<a.*?href=[\'\"]([^\"\']+)`)'); /* ".print_r($result[2], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	echo '</xmp>';
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = __uni_parseUniPath($unipath);
// print_r($result);
	$assert = "return \$result[2] == array('name' => 'sprintf1(`<a href=\"/%s/\" onclick=\"return load_album_into_player(%i, \'%s\', \'%s\');\"`, post_name, ID, post_title, guid)', 'unipath' => '/row/sprintf1(`<a href=\"/%s/\" onclick=\"return load_album_into_player(%i, \'%s\', \'%s\');\"`, post_name, ID, post_title, guid)'); /* ".print_r($result[2], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	echo '</xmp>';
// $GLOBALS['unipath_debug_parse'] = false;
...
// $GLOBALS['unipath_debug_parse'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = __uni_parseUniPath($unipath);
	$assert = "return \$result[1]['name'] == 'soxp_db()'; /* ".print_r($result[1], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = __uni_parseUniPath($unipath);
// var_dump($result[1]);
	$assert = "return \$result[2]['name'] == 'HTTP_HOST`'; /* ".print_r($result[2]['name'], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$result[3]['name'] == '\"1\"`2\'3`'; /* ".print_r($result[3]['name'], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$result[4]['name'] == '0'; /* ".print_r($result[4]['name'], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug_parse'] = false;
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = __uni_parseUniPath($unipath);
// var_dump($result[1]);
	$assert = "return \$result[1]['name'] == 'sprintf1(`````````it`s only test %s!```````, /if([\'`1\'=```1```` and /array(``, ```1]][2))`3```, \'\"]\')]/```test````,`1`,```2```)/0)'; /* ".print_r($result[1]['name'], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug_parse'] = false;
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = __uni_parseUniPath($unipath);
// var_export($result[1]);
	$assert = "return \$result[1]['filter'] == array(
			'start_expr' => 'expr1',
			'expr1' => array (
				'braket_level' => 0,
				'left_type' => 'string',
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = __uni_parseUniPath($unipath);
	$assert = "return \$result[3]['filter'] == array (
		'start_expr' => 'expr1',
		'expr1' => 
			array (
			'braket_level' => 0,
...
// $GLOBALS['unipath_debug_parse'] = false;
	...
	echo "<h3>--- $func_string ---</h3>";
	$result = __uni_parseFuncArgs($func_string);
	$assert = 'return $result == array(array(), array()); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
// $GLOBALS['unipath_debug'] = false;
	$assert = 'return $result == array(array("тов\'ар", "тов\"ара", "тов\'\'ар\"\"\"ов", "`тов\'\'ар\"\"\"ов`", "``тов\'\'ар\"\"\"о``в``", "тов\'\'ар\"\"\"о``в`", "```товар```", "товар"), array("string", "string", "string", "string", "string", "string", "string", "string")); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
	$assert = 'return $result == array(array("file://./unipath_test.json"), array("string")); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
	$assert = 'return $result == array(array("tmp_dir/asDirectory()/`tm_send.tmp`"), array("unipath")); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
	...
	echo "<h3>--- $func_string ---</h3>";
	$result = __uni_parseFuncArgs($func_string);
	$assert = "return \$result == array(array(\"db1/alias('default_Orchard_Framework_ContentItemRecord','cont')[ContentType_id=14 and Published=1]+alias('default_Orchard_Framework_ContentItemVersionRecord','vers')[cont.Id = vers.ContentItemRecord_id and Latest=1]
	+alias('default_Common_CommonPartRecord', 'comm')[comm.Id = cont.Id]
	+alias('default_Orchard_Autoroute_AutoroutePartRecord', 'urls')[urls.Id = vers.Id]
...
//$GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
	$assert = "return \$result == array(array('60' => 66, 'Хостинский' => 'Хостинский район', 'tmp' => './tmp'), array('60' => 'number', 'Хостинский' => 'string', 'tmp' => 'unipath')); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
// var_export($result);
	$assert = "return \$result == array(0 => array (
    'prd_code1c' => 'Code',
    'prd_name' => 'Description',
    'parent_code1c' => './`Родитель--Код`',
    'prd_deleted' => 'DeletionMark/replace_string(`ложь`=0,`истина`=1)/php_intval()',
...
// $GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
// var_export($result);
	$assert = "return \$result == array(array(0 => 'Data', 'test1' => '%abc%', 1 => '%abc%'), array(0 => 'unipath', 'test1' => 'string-with-N', 1 => 'string-with-N')); /* ".var_export($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = __uni_parseFuncArgs($func_string);
// var_export($result);
	$assert = "return \$result == array(array(0 => '[sprintf1(`%s %s`, name, last_name) = `Тест Тестович`]', 1 => ' selected=\"selected\"', 2 => ''), array(0 => 'unipath', 1 => 'string', 2 => 'string')); /* ".var_export($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
if(isset($_GET['test_uniPath'])) {
	...
	$tmp = uni($unipath);
	$assert = 'return count($tmp) == 2; /* '.json_encode($tmp).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tmp = array(0 => array('a' => 1), 1 => array('a' => 1, 'c' => 3)); /* ".print_r($tmp, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	...
	echo "<h3>--- $unipath ---</h3>";
	$tmp = uni($unipath);
	$assert = 'return count($tmp) == 2; /* '.print_r($tmp, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$tmp = array(0 => array('a' => 1), 1 => array('a' => 1, 'c' => 3)); /* ".print_r($tmp, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$tmp = uni($unipath);
	$assert = 'return count($tmp) == 0; /* '.json_encode($tmp).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $tmp == array(); /* '.json_encode($tmp).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$ret = uni($unipath, 999);
	$assert = 'return $ret == 999; /* '.print_r($ret, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$GLOBALS['test1'][0]['a'] == 999; /* ".print_r($GLOBALS['test1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
//$GLOBALS['unipath_debug'] = true;
	...
	$result = uni($unipath);
	$assert = 'return $result == "RSV-1_(2013_-_2_kvartal)(001).ods"; /* '.var_export($result,true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	$result = uni($unipath);
	$assert = 'return $result == "РСВ-1 (2013-2 квартал) 001.одс;%20"; /* '.var_export($result,true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);

	$unipath = 'formatDate()';
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == "'.strtr(date('j-m-Y, H:i'), array('-01-' => ' января ','-02-' => ' февраля ','-03-' => ' марта ', '-04-' => ' апреля ', '-05-' => ' мая ', '-06-' => ' июня ', '-07-' => ' июля ', '-08-' => ' августа ', '-09-' => ' сентября ', '-10-' => ' октября ', '-11-' => ' ноября ', '-12-' => ' декабря ')).'"; /* '.var_export($result,true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = null;
	$result = __uni_with_start_data($result, null, null, $unipath);
	$assert = 'return $result = array("abc" => 123); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$ret = uni($unipath, 123);
	$assert = 'return $ret == 123; /* '.print_r($ret, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS["post_cache1"] == array("abc" => 123); /* '.@print_r($GLOBALS['post_cache1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS_metadata["post_cache1"][0] == "array"; /* '.@print_r($GLOBALS_metadata["post_cache1"],true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == 123; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS["post_cache1"] == array("abc" => 123); /* '.@print_r($GLOBALS['post_cache1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS_metadata["post_cache1"][0] == "array"; /* '.print_r($GLOBALS_metadata["post_cache1"],true).' */';
...
// $GLOBALS['unipath_debug'] = true;
	...
	echo "<h3>--- $unipath = 123 ---</h3>";
	uni($unipath, '123');
	$assert = 'return file_get_contents("unipath_test.txt") == "123"; /* '.@file_get_contents("unipath_test.txt").' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$file_content = uni($unipath.'/contents()');
	$assert = 'return $file_content == "123"; /* '.print_r($file_content,true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == NULL; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $result == array("abc" => 123); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return file_get_contents("unipath_test.json") == \'{"name":"_POST","unipath":"\/_POST","data":{"abc":123},"metadata":{"0":"array","key()":"_POST"},"cache_timestamp":'.time().'}\'; /* '.@file_get_contents("unipath_test.json").' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == \'{"name":"_POST","unipath":"\/_POST","data":{"abc":123},"metadata":{"0":"array","key()":"_POST"},"cache_timestamp":'.time().'}\'; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == 3; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return isset($GLOBALS["uncached_var1"]); /* '.print_r($GLOBALS['uncached_var1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS_metadata[\'uncached_var1\'][0] == "array"; /* '.@print_r($GLOBALS_metadata['uncached_var1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == 2; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	echo "<h3>--- $unipath ---</h3>";
	$assert = 'return 1==2; /* Запись курсора в кеш не поддерживается! Запись в файл-курсор? */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	echo "<h3>--- $unipath ---</h3>";
	$assert = 'return 1==2; /* после успешного кеширования не возвращает рузультат! */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $result == array("abc" => 123); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return isset($GLOBALS["_post"]); /* '.print_r($GLOBALS['_post'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS_metadata[\'_post\'][0] == "array"; /* '.@print_r($GLOBALS_metadata['_post'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result = '<h1>123</h1>
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$result = uni($unipath);
	$assert = "return \$result = '<span class=\"obj-nophoto\">Нет фото</span>'; /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result = '123 руб.'; /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == '1 250 руб.'; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == '<span class=\"obj-photo\"><img src=\"/Media/Default/newbilding/mainf/Фотография3-1.jpg\" alt=\"\" width=\"80\" /></span>'; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == 'Media/Default/newbilding/mainf/Фотография3-1.jpg123'; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == 'Хостинский, 60 лет ВЛКСМ'; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result = "Хостинский район, 66 лет ВЛКСМ"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return defined('TEST_CONST');";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return is_object(\$result) == false; /* ".var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$result == 'test123'; /* ".var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return class_exists(TEST_CONST) == false; /* ".var_export(class_exists(TEST_CONST), true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;

	eval('namespace NameSpace1\NameSpace2; class TestClass {};');
	$unipath = '/NameSpace1/NameSpace2/TestClass/__construct()';
	echo "<h3>--- $unipath ---</h3>";
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return class_exists('\NameSpace1\NameSpace2\TestClass');";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return gettype(\$result) == 'object'; /* ".var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return is_object(\$result) && get_class(\$result) == 'NameSpace1\NameSpace2\TestClass'; /* ".(is_object($result)?get_class($result):var_export($result, true)).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$GLOBALS_metadata['tmp']['img_info'][0] == 1936;";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$GLOBALS_metadata['tmp']['img_info'][1] == 2592;";
	if(!eval($assert)) { trigger_error($assert, E_USER_WARNING); var_dump($result, $GLOBALS['tmp'], $GLOBALS_metadata['tmp']); }
// 	uni("url(`file://./IMG_5020.JPG`)/asImageFile()/fixOrientation()/saveAs('temp.jpg')");
	
	echo "<h3>--- new PDO('sqlite::memory:') ---</h3>";
	$result = new PDO('sqlite::memory:');
	$assert = "return isset(\$result) && is_object(\$result); /* ".var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = "return is_object(\$GLOBALS['db1']); /* ".var_export($GLOBALS['db1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return is_object(\$GLOBALS['db1']) && get_class(\$GLOBALS['db1']) == 'PDO'; /* ".print_r($GLOBALS['db1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
//$GLOBALS['unipath_debug'] = true;
	...
	$result = $result['.'];
	$assert = 'return is_object($result) && get_class($result) == "PDO"; /* '.print_r(is_object($result)?get_class($result):gettype($result), true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$sql = uni($unipath);
	$assert = 'return $sql == array("sql_query" =>"SELECT * FROM table1 WHERE id = -123", "sql_params" => array()); /* '.print_r($sql, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$sql = uni($unipath);
	$assert = 'return $sql == array("sql_query" =>"SELECT * FROM table1 WHERE 1", "sql_params" => array()); /* '.print_r($sql, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$sql = uni($unipath);
	$assert = 'return $sql == array("sql_query" => "SELECT * FROM table1", "sql_params" => array()); /* '.print_r($sql, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$sql = uni($unipath);
	$assert = 'return $sql == array("sql_query" => "SELECT * FROM table1 WHERE val LIKE \'%123%\'", "sql_params" => array()); /* '.print_r($sql, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$sql = uni($unipath);
	$assert = 'return $sql == array("sql_query" => "SELECT * FROM table1 WHERE val IS NULL", "sql_params" => array()); /* '.print_r($sql, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	if(isset($GLOBALS['db1']))
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array(array(
		'cat_id' => '1',
		'cat_parent_id' => '0',
		'cat_name' => 'Category 1',
		'cat_url' => NULL,
...
// $GLOBALS['unipath_debug'] = true;
	$result = null;
	$result = __uni_with_start_data($result, null, null, $unipath, '00000000002');
	$result = $GLOBALS['db1']->query("SELECT * FROM tbl_categories WHERE cat_id = 2")->fetchAll(PDO::FETCH_ASSOC);
	$assert = 'return $result[0]["cat_code1c"] === "00000000002"; /* '.print_r($result[0], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $result[0]["cat_code1c"] === "00000000002"; /* '.print_r($result[0], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS["unipath_debug_sql"] == array("UPDATE tbl_categories SET cat_code1c = \'00000000002\' WHERE cat_id = 2"); /* '.print_r($GLOBALS['unipath_debug_sql'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$GLOBALS['unipath_debug_sql'] = array();
	$result = null;
	$result = __uni_with_start_data($result, null, null, $unipath, array('cat_code1c' => '00000000002'));
	$result = $GLOBALS['db1']->query("SELECT * FROM tbl_categories WHERE cat_id = 2")->fetchAll(PDO::FETCH_ASSOC);
	$assert = 'return $result[0]["cat_code1c"] === "00000000002"; /* '.print_r($result[0], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $GLOBALS["unipath_debug_sql"] == array("UPDATE tbl_categories SET cat_code1c = \'00000000002\' WHERE cat_id = 2"); /* '.print_r($GLOBALS['unipath_debug_sql'], true).' */';
...
// 	$assert = 'return strpos($error["message"], "Data not changed.") > 0; /* '.print_r($error, true).' */';
//	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$rows = $GLOBALS['db1']->query("SELECT * FROM tbl_categories WHERE cat_id = 2")->fetchAll(PDO::FETCH_ASSOC);
// 	$assert = 'return $rows[0]["cat_code1c"] === "CODE0000002"; /* '.print_r($rows[0], true).' */';
//	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = null;
	$result = __uni_with_start_data($result, null, null, $unipath);
	$assert = 'return $result["data"] === "0" || $result["data"] === 0; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result["metadata"][0] === is_string($result["data"]) ? "string" : "integer"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result["metadata"]["key()"] === "cat_deleted"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$GLOBALS['unipath_debug_sql'] = array();
	uni($unipath, 1);
	$assert = 'return $GLOBALS["unipath_debug_sql"] == array(
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $GLOBALS["unipath_debug_sql"] == array("UPDATE tbl_categories SET cat_deleted = 2 WHERE cat_id = 2"); /* '.print_r($GLOBALS["unipath_debug_sql"], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	$rows = $GLOBALS['db1']->query("SELECT cat_deleted FROM tbl_categories")->fetchAll(PDO::FETCH_ASSOC);
	$assert = 'return $rows == array(array("cat_deleted" => 0), array("cat_deleted" => 2)); /* '.print_r($rows, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	unset($GLOBALS['unipath_debug_sql']);
// $GLOBALS['unipath_debug'] = false;

	$unipath = "/db1/tbl_categories[cat_id=".join(',', array(1,2,3))." and cat_deleted=1]";
	echo "<h3>--- $unipath = array('cat_deleted' => 2) ---</h3>";
// $GLOBALS['unipath_debug'] = true;
	$GLOBALS['unipath_debug_sql'] = array();
	uni($unipath, array('cat_deleted' => 2));
	$assert = 'return $GLOBALS["unipath_debug_sql"] == array("UPDATE tbl_categories SET cat_deleted = 2 WHERE cat_id IN (1,2,3) and cat_deleted = 1"); /* '.print_r($GLOBALS["unipath_debug_sql"], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => 'SELECT * FROM tbl_categories LEFT OUTER JOIN tbl_categories_stat ON tbl_categories_stat.cat_id = tbl_categories.cat_id WHERE cat_hidden = 0', 'sql_params' => array()); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	uni($unipath);
// $GLOBALS['unipath_debug'] = false;
	$assert = "return is_array(\$GLOBALS_metadata['db1_cats']); /* ".print_r($GLOBALS_metadata['db1_cats'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return !isset(\$GLOBALS_metadata['db1_cats']['stmt']);";
	if(!eval($assert)) { trigger_error($assert, E_USER_WARNING); print_r($GLOBALS_metadata['db1_cats']); }
	// ???
	$assert = "return !empty(\$GLOBALS['tmp_rows_cache1']); /* ".@print_r($GLOBALS['tmp_rows_cache1'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => 'SELECT * FROM tbl_categories WHERE date < NOW()', 'sql_params' => array()); /* ".@print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => 'SELECT * FROM tbl_categories WHERE id NOT IN (1,2,3) and id NOT IN (4,5,6) and cat_name NOT IN (\'aaa\', \'bbb\', \'ccc\') and cat_name NOT IN (\'eee\', \'fff\')', 'sql_params' => array()); /* ".@print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return count($result) == 2; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// print_r($GLOBALS_metadata['db1_cats']);
	$assert = "return count(\$GLOBALS_metadata['db1_cats']['result_cache']) == 2; /* ".print_r($GLOBALS_metadata['db1_cats']['result_cache'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$GLOBALS['tmp_rows_cache1'] == array (
	0 => 
	array (
		'cat_id' => '1',
		'cat_parent_id' => '0',
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return current(\$result) == array(
		'cat_id' => '2',
		'cat_parent_id' => '0',
		'cat_name' => 'Category 2',
		'cat_url' => NULL,
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('CODE0000001' => 'Category 1'); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return get_class($result) == "Uni"; /* '.print_r(get_class($result), true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return empty($GLOBALS["tmp_rows_cache1"]); /* '.@print_r($GLOBALS["tmp_rows_cache1"], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = "return !empty(\$result_rewind); /* ".var_export($result_rewind, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = false;
	$assert = "return \$result_valid == true; /* ".var_export($result_valid, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);

	echo "<h3>--- \$uni->current() ---</h3>";
	$result_current = $result->current();
	$assert = "return \$result_current == array(
		'cat_id' => '1',
		'cat_parent_id' => '0',
		'cat_name' => 'Category 1',
		'cat_url' => NULL,
	...
	$assert = "return \$result_key === 0; /* ".print_r($result_key, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	echo "<h3>--- \$uni->next() ---</h3>";
	$result_next = $result->next();
	$assert = "return \$result_next == array(
		'cat_id' => '2',
		'cat_parent_id' => '0',
		'cat_name' => 'Category 2',
		'cat_url' => NULL,
	...
	$result_key = $result->key();
	$assert = "return \$result_key === 1; /* ".var_export($result_key, true).' ('.var_export($result->current_cursor_result['metadata'], true).') */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	echo "<h3>\$result->next()</h3>";
// 	$GLOBALS['unipath_debug'] = true;
	$result_next = $result->next();
	$assert = "return \$result_next == null; /* ".var_export($result_next, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$GLOBALS['unipath_debug'] = false;
	
	$result_current = $result->current();
	$assert = "return \$result_current == null; /* ".var_export($result_current, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	$result_key = $result->key();
	$assert = "return \$result_key === null; /* ".var_export($result_key, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	$result_rewind = $result->rewind();
	$assert = "return !empty(\$result_rewind); /* ".print_r($result_rewind, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// 	$GLOBALS['unipath_debug'] = true;
	...
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
				...
				$val2 = $val['cat_name'];
				$assert = 'return is_string($val2); /* '.print_r($val2, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
				break;
			case 1:
				$key1_found = true;
	...
	$assert = 'return $key0_found == true and $key1_found == true; /* '.var_export(array('key0_found' => $key0_found, 'key1_found' => $key1_found), true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = true;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "cat_id != 2"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// 	$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "Category 1 (ID:1), Category 2 (ID:2), "; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $rows == array(array("cat_id" => 2, "cat_parent_id" => 0)); /* '.print_r($rows, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$assert = 'return $result == "cat_id != 2"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	...
	$rows = $GLOBALS['db1']->query("SELECT cat_id, cat_parent_id FROM tbl_categories")->fetchAll(PDO::FETCH_ASSOC);
	$assert = 'return $rows == array(array("cat_id" => 2, "cat_parent_id" => 999)); /* '.print_r($rows, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	...
	$assert = 'return $rows == array(); /* '.print_r($rows, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result["test_list"] == array("~/Media/Default/newbilding/gallery/фото-1.jpg"," ~/Media/Default/newbilding/gallery/фото-2.jpg"," ","~/Media/Default/newbilding/gallery/фото-3.jpg "); /* '.print_r($result['test_list'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == array("открытый поселок"); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "(none)"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "yes"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == ""; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == 0; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug_parse'] = true;
	$result = uni($unipath);
	$assert = "return \$result = array('sql_query' => \"SELECT SUBSTRING(table1.Data, 1, 3000) AS uni_chunk_0_table1_Data, SUBSTRING(table1.Data, 3001, 3000) AS uni_chunk_1_table1_Data, SUBSTRING(table1.Data, 6001, 3000) AS uni_chunk_2_table1_Data, SUBSTRING(table1.Data, 9001, 3000) AS uni_chunk_3_table1_Data, table1.Id AS id1, table1.ContentType_id, REPLACE('abcd)asd', ')', '(') FROM table1 WHERE id = 1\", 'sql_binds' => array()); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
// $GLOBALS['unipath_debug_parse'] = false;
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('title' => 'Элитная недвижимость'); /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == '/hotel/'; /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
//$GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => \"SELECT * FROM default_Orchard_Framework_ContentItemRecord WHERE ContentType_id IN (11,14) and Published = 1 and (`Data` LIKE N'%abc%' or ContentItemRecord_id = 'abc' or Title LIKE N'%abc%')\", 'sql_params' => array()); /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => \"SELECT * FROM objects_for_admin WHERE obj_deleted = 0 and (`title` ILIKE '%{$search_query}%' or url ILIKE '%{$search_query}%' or city_part ILIKE '%{$search_query}%' or street ILIKE '%{$search_query}%')\", 'sql_params' => array()); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('newbilding' => 11, 'room' => 18, 'house' => 15, 'zemuch' => 14, 'hotel' => 28, 'office' => 27, 'invproject' => 29); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result = array("УТ0001039" => array("cat_code1c" => "УТ0001039", "cat_name" => "Бытовая техника", "parent_code1c" => "УТ0000883", "parent_name" => "FunBit", "cat_hidden" => 0)); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == array('sql_query' => \"SELECT * FROM users WHERE u_login = 'test' and u_password_hash = '123'\", 'sql_params' => array()); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	
	$unipath = "db1/contacts[PHONE1=php:bonuses_phone_variants(/_GET/LK_CONTACT_ID) or PHONE2=php:bonuses_phone_variants(/_GET/LK_CONTACT_ID)]";
	echo "<h3>--- $unipath ---</h3>";
	// TODO 'php:bonuses_phone_variants' not a SQL-function!

	$unipath = "/bx24_db()/contacts[like('%UF_CRM_TELEGRAMUSERNAME_WZ%', DATA)]/columns('ID')/*/ID";
	echo "<h3>--- $unipath ---</h3>";
	// TODO $result == array("1", "2", "3"...) not array(array("ID" => "1", "ID" => "2", "ID" => "3"...))

    $_POST = array('u_login' => 'test', 'u_password_hash' => 123, 'password2' => '123', 'submitted' => 1);
	$unipath = '$_POST/unset(`password2`)/unset(`submitted`)';
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == array('u_login' => 'test', 'u_password_hash' => 123); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == 1; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array('u_login' => 'test', 'u_password_hash' => 123, 'password2' => '123', 'submitted' => 1); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// echo '<xmp>';
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == array("u_password_hash" => 123, "password2" => "123"); /* '.json_encode($result).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
// echo '<xmp>';
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = "return \$result == array(0 => array('DataXML' => array('zemucharea' => 'Лазаревский', 'priceuch' => 0))); /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;
	echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
// print_r($result);
	$assert = 'return $result[0] instanceof DOMElement; /* '.var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result[0]->nodeName == "zemuch"; /* '.var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	echo '</xmp>';
...
// var_dump($result, $_POST);
	$assert = 'return $result == "/Media/Default/Avatars/6842.png"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// echo '</xmp>';
// unset($GLOBALS['unipath_debug']);
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$result = uni($unipath);
//print_r($result);
	$assert = 'return $result == "test_xml"; /* '.var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$result = uni($unipath);
//print_r($result);
	$assert = 'return $result == "3-комн квартира на ул Труда"; /* '.var_export($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 
	echo '</xmp>';
...
//$GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$result = uni($unipath);
//print_r($result);
	$assert = 'return $result == "Цена: 5800000 руб. "; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
//$GLOBALS['unipath_debug'] = false; echo '</xmp>';

	$unipath = '/_POST/test_xml/asXML()/h/0/@outline-level';
	echo "<h3>--- $unipath ---</h3>";
//$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == 1; /* '.print_r($result, true).' */';
...
//$GLOBALS['unipath_debug'] = false;
	...
	$unipath = '/_POST/test_xml/asXML()/h/0/@text:style-name';
	echo "<h3>--- $unipath ---</h3>";
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "Heading_20_1"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;

	$unipath = '/_POST/test_xml/asXML()/p/*/@style-name';
	echo "<h3>--- $unipath ---</h3>";
// $GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == array("Text_20_body", "Text_20_body", "Text_20_body", "Text_20_body", "Text_20_body", "Standard"); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false;

	$unipath = "/_POST/test_xml/asXML()/*/.[./nodeName='h','table']/all()";
	echo "<h3>--- $unipath ---</h3>";
// $GLOBALS['unipath_debug'] = true; 
echo '<xmp>';
	$result = uni($unipath);
// var_dump($result);
	$assert = 'return is_array($result) and count($result) == 2; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result[0]->nodeName == "h" && $result[0]->nodeValue == "3-комн квартира на ул Труда"; /* '.print_r($result[0], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result[7]->nodeName == "table" && $result[7]->nodeValue == "РайонЦентральныйУлицаТеатральная 11КласспремиумКол-во этажей9"; /* '.print_r($result[7], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 
echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == \$_POST['test_xml']; /* ".print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	uni($unipath);
	$assert = "return get_class(\$GLOBALS['odt']) == 'DOMElement'; /* ".print_r($GLOBALS['odt'], true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';

// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$unipath = 'cache($odt)/p/*/nodeValue';
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	try {
	$assert = "return count(\$result) == 7; /* ".print_r(count($result), true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	} catch(Error $ex) { trigger_error($assert, E_USER_WARNING); }
	$assert = 'return $result == array("Улица: Труда", "Общая площадь: 85 м² ", "Этажность: 12 ", "Этаж: 6 ", "Цена: 5800000 руб. ", "", ""); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	print_r($result);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; 
	$result = uni($unipath);
	$assert = "return count(\$result) == 4; /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = "return \$result[1]->nodeValue == 'УлицаТеатральная 11'; /* ".print_r($result[1], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 

	$unipath = 'cache($odt)/table/0/*/toHash(nodeName)';
	echo "<h3>--- $unipath ---</h3>";
// $GLOBALS['unipath_debug'] = true; 
// echo '<xmp>';
	$result = uni($unipath);
	$assert = "return isset(\$result['table-column'], \$result['table-row']); /* ".print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 
// echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; 
	$result = uni($unipath);
	$assert = 'return isset($result["style-name"], $result["outline-level"]); /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return count($result) == 9; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	$result = uni($unipath);
	$assert = 'return $result == "Data"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == \'<Data><newbilding><mainfnewb Width="200" Height="150" AlternateText="SAM_1591.JPG">~/Media/Default/newbilding/mainf/SAM_1591-5.JPG</mainfnewb><newbfotogal>/Media/Default/newbilding/gallery/SAM_1591-1.jpg</newbfotogal></newbilding></Data>\'; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
	...
	uni($unipath, 321);
	$assert = 'return $_POST[123] == 321; /* '.print_r($_POST, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	$assert = 'return $_POST["favorites"][123] == array("name" => "test"); /* '.print_r($_POST, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	...
	$assert = 'return $_POST["favorites"][123] == array("name" => "test2"); /* '.print_r($GLOBALS['_POST'], true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);

	$unipath = '_POST/favorites/333';
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return is_null($result); /* '.json_encode($result).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	$result = uni($unipath);
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	$assert = 'return $result == "<name>test2</name>"; /* '.print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; 
	echo '<xmp>';
	$result = uni($unipath);
	$assert = 'return $result == "<favorites><name>test2</name></favorites>"; /* '.print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; 
	echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == "name = test2"; /* '.print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == "test2 (name)"; /* '.print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = 'return $result == \'<img class="c-image_viewer-preload-image c-image_viewer-switcher" data-src="/images/940x350/6.jpg" alt=""><img class="c-image_viewer-preload-image c-image_viewer-switcher" data-src="/images/940x350/87.jpg" alt="">\'; /* '.print_r($result, true)." */";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == '$__test_our_url?echo=abc123'; /* ".print_r($result, true)." */ ";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == 'abc123'; /* ".print_r($result, true)." */ ";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// 	echo "<h3>--- $unipath ---</h3>";
// 	$result = uni($unipath);
// 	$assert = "return \$result == '1-komnatnie_kvartiry'; /* ".print_r($result, true)." */ ";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == array('image0' => '123.jpg', 'image999' => 'image999.png'); /* ".print_r($result, true)." */ ";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
// $GLOBALS['unipath_debug'] = true; echo '<xmp>';
	...
	echo "<h3>--- $unipath ---</h3>";
	$result = uni($unipath);
	$assert = "return \$result == array('image0' => '<b>123.jpg</b>', 'image999' => '<b>image999.png</b>'); /* ".print_r($result, true)." */ ";
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// $GLOBALS['unipath_debug'] = false; echo '</xmp>';
...
/*	$xml = '<rows>
	...
	echo "<h3>--- $unipath ---</h3><xmp>";
	$result = uni($unipath);
	$assert = 'return $result == "<xml><aaa>111</aaa><bbb>222</bbb><ccc>333</ccc></xml>"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
	echo '</xmp>';
...
// 	$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "stdClass" || $result == "InternalIterator"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$GLOBALS['unipath_debug'] = false;
...
// 	$GLOBALS['unipath_debug'] = true;
	$result = uni($unipath);
	$assert = 'return $result == "class stdClass;" || $result == "class InternalIterator;"; /* '.print_r($result, true).' */';
	if(!eval($assert)) trigger_error($assert, E_USER_WARNING);
// 	$GLOBALS['unipath_debug'] = false;
